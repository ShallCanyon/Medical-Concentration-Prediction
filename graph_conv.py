...
def param_tuning(train_dataset, valid_dataset):
    def model_builder(**model_params):
        ...
        model = AttentiveFPModel(mode='regression', n_tasks=1, num_timesteps=2,
                                 # dropout=dropout,
...
def train_models(X, y, epoch=5):
    ...
    for i in range(epoch):
        ...
        train_scores, valid_scores, model, metric_list = \
            train_GCNModel(train_dataset, valid_dataset, lr=lr[0], model_dir=model_dir)
        test_scores = model.evaluate(test_dataset, metric_list)
        logger.info(f"Model{i} result:")
        logger.info(f"Learning rate:{lr[0]}")
...
if __name__ == '__main__':
    ...
    _, blood_y, brain_y, ratio_y, SMILES = get_X_Y(cfg.padel_csvfilepath)
    X = featurize(SMILES, is_torch=True)
    y = ratio_y
    train_models(X, y)

    # pred_dataset = data.NumpyDataset(X=X, y=y)
    # load_model_dir = "Models/20230120014437/model0"
    # score = predict_on_model(load_model_dir, pred_dataset)
    # logger.info(f"Predict score on model - {load_model_dir} :\n\t", score)
